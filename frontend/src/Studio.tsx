...
function TakeCard({take,onPreferred,admin=false}:{take:Take;onPreferred:()=>Promise<void>;admin?:boolean}){
 const [busy,setBusy]=useState(false);const [error,setError]=useState('');
 async function prefer(){setBusy(true);setError('');try{await request(`/api/actor/takes/${take.id}/preferred`,'PUT',{preferred:true});await onPreferred();}catch(e){setError(errorMessage(e));}finally{setBusy(false);}}
 return <article className="take-card"><div className="take-heading"><strong>Take {take.take_number}</strong><span>{new Date(take.created_at).toLocaleString()} · {(take.duration_ms/1000).toFixed(1)}s</span>{take.stale&&<span className="old-take">Earlier script version</span>}</div><audio controls preload="none" src={`/api/actor/takes/${take.id}/audio`} aria-label={`Play take ${take.take_number}`} onPlay={e=>{const current=e.currentTarget;document.querySelectorAll('audio').forEach(a=>{if(a!==current)a.pause();});}}/><>{admin?<button className={take.preferred?'preferred':'secondary'} disabled={busy||take.preferred} onClick={prefer}>{take.preferred?'★ Preferred take':'☆ Make preferred'}</button>:take.preferred?<span className="preferred">★ Preferred take</span>:null}</>{take.stale&&<details><summary>Words recorded for this take</summary><p>{take.text}</p>{take.direction&&<p>{take.direction}</p>}</details>}{error&&<p role="alert">{error}</p>}</article>;
}
...
function ActorStudio({session,onAdmin,actors,onActor}:{session:Session;onAdmin:()=>void;actors?:Named[];onActor?:(actor:Named)=>void}){
 ...
 {history?<section><h2>Your performances</h2>{!ownTakes.length&&<p>No saved takes yet. Your first performance can start in My scenes.</p>}{ownTakes.map(t=><div key={t.id} className="history-entry"><h3>{t.project_name} · {t.scene_name} · {t.character_name}</h3><p>{t.text}</p><TakeCard take={t} admin={session.user.admin} onPreferred={loadTakes}/></div>)}</section>:!scene?<div className="scene-cards">{!data.scenes.length&&<div className="studio-empty"><h2>A part is on its way.</h2><p>Once your character has dialogue, your scenes will appear here.</p></div>}{data.scenes.map(s=>{const part=mine.filter(l=>l.scene_id===s.id),done=part.filter(saved).length;return <button className="scene-card" key={s.id} onClick={()=>{setSceneID(s.id);setLineID(part.find(l=>!saved(l))?.id||part[0]?.id||'');}}><span>{data.projects.find(p=>p.id===s.project_id)?.name}</span><h2>{s.name}</h2><p>{Array.from(new Set(part.map(l=>char(l.character_id)))).join(' · ')}</p><progress value={done} max={part.length||1}/><strong>{done===part.length?'All lines have a take!':`${part.length-done} lines waiting for your voice`}</strong><small>{done} of {part.length} lines recorded</small></button>})}</div>:line?<>
 ...
 <Voicing sceneID={scene.id} admin={session.user.admin} eventID={line.id} version={JSON.stringify([context,takes])} disabled={recordingDirty}/>
